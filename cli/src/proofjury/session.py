...
``proofjury run <kind> -- <cmd>`` stamps ``.proofjury/session.json`` with
...
def session_path(root: Path) -> Path:
    return Path(root) / ".proofjury" / "session.json"
...
def _drop_proofjury_status_lines(status: str) -> str:
    """Proofjury's own state dir must never count as 'code changed'."""
    return "\n".join(
        line for line in status.splitlines() if ".proofjury" not in line
    )


def _drop_proofjury_diff_sections(diff: str) -> str:
...
def worktree_digest(root: Path) -> str:
    ...
    ``.proofjury/`` itself is excluded — gate bookkeeping is not code.
    """
    root = Path(root)
    try:
        ...
        status = _drop_proofjury_status_lines(status)
        ...
        diff = _drop_proofjury_diff_sections(diff)
        h = hashlib.sha1()
        ...
        for relpath in sorted(_untracked_paths_from_status(status)):
            if ".proofjury" in Path(relpath).parts:
                continue
            f = root / relpath
            try:
...
def stamp(root: Path, kind: str, exit_code: int, cmd: list[str]) -> dict:
    ...
    root = Path(root)
    # Create .proofjury/ BEFORE computing the digest so the untracked-dir
