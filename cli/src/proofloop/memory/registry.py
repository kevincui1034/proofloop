"""User-level registry of proofloop stores — memory recall across repos.

A machine accumulates one ``.proofloop/`` store per gated repo. The
...
at ``${XDG_CONFIG_HOME:-~/.config}/proofloop/registry.json``, refreshed
on every gate run.
