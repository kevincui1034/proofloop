...
def load_config(root: Path) -> dict:
    """Parse ``.proofloop.toml`` if present; never raise."""
    path = root / ".proofloop.toml"
    if not path.is_file():
        return {}
    try:
...
def resolve_repo_id(root: Path) -> str:
    ...
    ``.proofloop/repo_id`` on the first gate run; read back thereafter,
    so adding/removing the origin remote never orphans prior memory
    records for recall.
    """
    path = Path(root) / ".proofloop" / "repo_id"
    try:
