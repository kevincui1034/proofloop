// REAL transcripts — transcribed from an actual `proofjury guard` run of the
// §3 acceptance scenario (scripts/demo.sh, proofjury 0.1.0, deterministic
...
export const gateStoryScript: TerminalLine[] = [
  { type: "cmd", text: "proofjury guard deploy -- ./deploy.sh" },
  { type: "status", text: "⛔ DEPLOY BLOCKED — proofjury", tone: "red", delayMs: 500 },
  ...
  { type: "out", text: "  2. Run: proofjury run tests -- pytest" },
  { type: "out", text: "  3. Point config at production values; disable debug flags" },
  { type: "out", text: "record chk_001 → .proofjury/memory.jsonl · proof: .proofjury/runs/chk_001/ · exit 2", tone: "dim", delayMs: 300 },
  // — the agent applies the listed fixes —
  { type: "cmd", text: "export STRIPE_API_KEY=••••• DATABASE_URL=•••••", delayMs: 900 },
  { type: "cmd", text: "proofjury run tests -- pytest -q", delayMs: 300 },
  { type: "out", text: "4 passed in 0.01s", tone: "green", delayMs: 450 },
  { type: "out", text: "proofjury: recorded tests run (exit 0) → .proofjury/runs/", tone: "dim" },
  // — re-run the gate —
  { type: "cmd", text: "proofjury guard deploy -- ./deploy.sh", delayMs: 500 },
...
export const recurrenceScript: TerminalLine[] = [
  { type: "cmd", text: "proofjury guard deploy -- ./deploy.sh" },
  { type: "status", text: "⛔ DEPLOY BLOCKED — proofjury", tone: "red", delayMs: 450 },
  ...
  { type: "out", text: "record chk_003 → .proofjury/memory.jsonl · proof: .proofjury/runs/chk_003/ · exit 2", tone: "dim", delayMs: 300 },
];
